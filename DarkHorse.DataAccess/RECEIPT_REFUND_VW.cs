...
namespace DarkHorse.DataAccess
{
    public class ReceiptRefund : BaseTableClass
    {
        ...
        public static async Task<ReceiptRefund> GetSingleReceiptAsync(int recieptId, IDbConnection dbConnection)
        {
            if (dbConnection is SqlConnection)
            {
                using var connection = new SqlConnection(dbConnection.ConnectionString);
                ...
                var sql = $@"SELECT RECEIPT_BATCH_ID,
                                  ...
                                  CANCEL_CASHIER_ID
                                FROM RECEIPT_REFUND_VW
                                WHERE RECEIPT_ID = {recieptId}
                                ORDER BY CREATED_DT DESC";

                return await connection.QueryFirstOrDefaultAsync<ReceiptRefund>(sql).ConfigureAwait(false);
            }
            else
            {
                using var connection = new OracleConnection(dbConnection.ConnectionString);
